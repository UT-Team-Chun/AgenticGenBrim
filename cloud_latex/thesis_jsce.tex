...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw,rounded corners,align=center,inner sep=4pt,minimum height=8mm,text width=22mm},
  lab/.style={font=\scriptsize,fill=white,inner sep=1.5pt},
  arrow/.style={-Latex,thick},
  node distance=8mm and 12mm
]
  \node[box] (input) {入力\\橋長 $L$\\幅員 $B$};
  \node[box, right=of input] (rag) {RAG\\検索};
  \node[box, right=of rag] (designer) {Designer\\(設計生成)};
  \node[box, right=of designer] (judge) {Judge\\(照査)};
  \node[box, right=of judge] (ifc) {IFC変換};
  \node[box, right=of ifc] (viewer) {BIM\\ビューア};

  \node[box, above=12mm of rag] (pdf) {PDF\\知識ベース};
  \node[box, above=12mm of designer] (openai) {OpenAI API};

  \draw[arrow] (input) -- (rag);
  \draw[arrow] (rag) -- node[above,lab]{コンテキスト} (designer);
  \draw[arrow] (designer) -- node[above,lab]{設計JSON} (judge);
  \draw[arrow] (judge) -- node[above,lab]{合格} (ifc);
  \draw[arrow] (ifc) -- (viewer);

  \draw[arrow] (pdf) -- (rag);
  \draw[arrow] (openai) -- (rag);
  \draw[arrow] (openai) -- (designer);
  \draw[arrow] (openai) -- (judge);

  % Judge repair loop
  \draw[arrow,dashed] (judge.south) -- ++(0,-8mm) node[below,lab]{不合格} -| node[near start,below,lab]{PatchPlan} (designer.south);
\end{tikzpicture}
}%
