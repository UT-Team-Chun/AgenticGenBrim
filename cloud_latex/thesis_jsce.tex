...
図\ref{fig:raglog_example}に，実際のRAG検索ヒット結果の例を示す．各ヒットにはrank番号，コサイン類似度スコア（score），出典ドキュメント名（source），ページ番号（page），および抽出されたテキスト（text）が含まれる．この例では，「鋼橋設計の基本\_第七章 プレートガーダー橋.pdf」や「鋼橋設計の基本\_第一章 概論.pdf」など複数の文献から，プレートガーダー橋の設計に関連するチャンクが類似度順に取得されている．Designerはこれらのヒットを参照しながら設計値を決定し，採用した根拠のrank番号をsource\_hit\_ranksとして記録する．
...
{
  "rank": 3, "score": 0.694,
  "source": "鋼橋設計の基本_第七章 プレートガーダー橋.pdf",
  "page": 0,
...
LLMにはgpt-5.1を使用した．Designer--Judgeの修正ループにおける最大イテレーション数は5回とした．すなわち，Designerが初期設計を生成した後，Judgeによる照査で不合格となった場合，最大5回まで修正と再照査を繰り返す．5回の修正で合格に至らなかった場合は不合格として記録する．各条件について3試行を実施しているのは，LLMの出力の確率的な変動を考慮し，結果の安定性を評価するためである．
...
\begin{itemize}
  ...
  \item \textbf{収束までのイテレーション数}：合格に至った試行について，Designer--Judgeの修正ループが何回で収束したかを集計する．イテレーション数が少ないほど，初期設計の品質が高いことを示す．
\end{itemize}
...
表\ref{tab:check_item_pass_rate}にチェック項目別の初回合格率を示す．shear（せん断）およびweb\_slenderness（腹板幅厚比）は全ケースで初回合格率100\%であった．これは，本評価の橋長・幅員範囲（$L=20$--$70$\,m, $B=8$--$24$\,m）の単純桁においてせん断応力が支配的照査項目とならないことと整合する．deck（床版厚）はRAGありで100\%，RAGなしで66.7\%であった．deflection（たわみ）はRAGの効果が最も顕著であり，RAGありで51.0\%に対しRAGなしで6.2\%と，44.8ポイントの差が生じた．bend（曲げ）はRAGありで46.9\%，RAGなしで13.5\%であった．
...
表\ref{tab:by_length}に橋長別の結果を示す．RAGありではL=45\,mで最高の初回合格率66.7\%を記録した．長スパン（$L=65$--$70$\,m）でもRAGありは初回合格率44--56\%を維持している一方，RAGなしは全橋長で初回合格率0\%であった．収束率は橋長が長くなるほど低下する傾向があり，特にRAGなしでは$L=65$\,mで11\%まで低下した．
...
\hline
L=20\,m & 8, 10 & あり & 0.0\% & 1.83 & 100\% \\
\hline
L=20\,m & 8, 10 & なし & 0.0\% & 1.83 & 100\% \\
\hline
L=25\,m & 8, 10, 12 & あり & 22.2\% & 1.56 & 100\% \\
\hline
L=25\,m & 8, 10, 12 & なし & 0.0\% & 2.11 & 100\% \\
\hline
L=30\,m & 8, 10, 12 & あり & 33.3\% & 1.33 & 100\% \\
\hline
L=30\,m & 8, 10, 12 & なし & 0.0\% & 1.56 & 100\% \\
\hline
L=35\,m & 8, 10, 16 & あり & 22.2\% & 1.33 & 100\% \\
\hline
L=35\,m & 8, 10, 16 & なし & 0.0\% & 1.56 & 100\% \\
\hline
L=40\,m & 8, 10, 20 & あり & 33.3\% & 1.57 & 78\% \\
\hline
L=40\,m & 8, 10, 20 & なし & 0.0\% & 2.56 & 100\% \\
\hline
L=45\,m & 8, 10, 20 & あり & 66.7\% & 1.25 & 89\% \\
\hline
L=45\,m & 8, 10, 20 & なし & 0.0\% & 2.43 & 78\% \\
\hline
L=50\,m & 10, 16, 24 & あり & 44.4\% & 1.57 & 78\% \\
\hline
L=50\,m & 10, 16, 24 & なし & 0.0\% & 3.14 & 78\% \\
\hline
L=55\,m & 10, 16, 24 & あり & 44.4\% & 2.00 & 100\% \\
\hline
L=55\,m & 10, 16, 24 & なし & 0.0\% & 3.00 & 67\% \\
\hline
L=60\,m & 10, 16, 24 & あり & 33.3\% & 1.50 & 67\% \\
\hline
L=60\,m & 10, 16, 24 & なし & 0.0\% & 4.20 & 56\% \\
\hline
L=65\,m & 12, 16, 24 & あり & 55.6\% & 0.57 & 78\% \\
\hline
L=65\,m & 12, 16, 24 & なし & 0.0\% & 5.00 & 11\% \\
\hline
L=70\,m & 12, 16, 24 & あり & 44.4\% & 0.60 & 56\% \\
\hline
L=70\,m & 12, 16, 24 & なし & 0.0\% & 4.50 & 44\% \\
\hline
\end{tabular}
\end{table*}

表\ref{tab:by_width}に幅員別の結果を示す．$B=8$\,mではRAGありの初回合格率が66.7\%と最高であり，$B=8$--$16$\,mではRAGありの収束率は94--100\%であった．一方，$B=20$--$24$\,mでは難易度が顕著に上昇し，$B=24$\,mではRAGなしの収束率が7\%（15件中1件のみ合格）まで低下した．$B=20$\,mではRAGなしの収束率がRAGありを上回る逆転現象（67\% vs 50\%）が観測された．
...
\hline
B=8\,m & 20, 25, 30, 35, 40, 45 & あり & 66.7\% & 0.56 & 100\% \\
\hline
B=8\,m & 20, 25, 30, 35, 40, 45 & なし & 0.0\% & 1.50 & 100\% \\
\hline
B=10\,m & 20--60 & あり & 40.7\% & 1.15 & 100\% \\
\hline
B=10\,m & 20--60 & なし & 0.0\% & 2.33 & 100\% \\
\hline
B=12\,m & 25, 30, 65, 70 & あり & 41.7\% & 1.25 & 100\% \\
\hline
B=12\,m & 25, 30, 65, 70 & なし & 0.0\% & 3.00 & 75\% \\
\hline
B=16\,m & 35, 50--70 & あり & 38.9\% & 1.76 & 94\% \\
\hline
B=16\,m & 35, 50--70 & なし & 0.0\% & 3.38 & 72\% \\
\hline
B=20\,m & 40, 45 & あり & 0.0\% & 4.67 & 50\% \\
\hline
B=20\,m & 40, 45 & なし & 0.0\% & 4.00 & 67\% \\
\hline
B=24\,m & 50--70 & あり & 6.7\% & 3.00 & 33\% \\
\hline
B=24\,m & 50--70 & なし & 0.0\% & 5.00 & 7\% \\
\hline
\end{tabular}
\end{table*}
...
不収束ケースの不合格項目内訳を以下に示す．bendのみが22件（57.9\%），bend+web\_slendernessが13件（34.2\%），web\_slendernessのみが3件（7.9\%）であった．deck，shear，deflectionによる不収束は0件であった．
...
表\ref{tab:rag_param_effect}に，初回設計（修正ループ前）における主要断面パラメータのRAG有無による比較を示す．32条件それぞれでRAGあり/なし各3試行の平均を1データ点とし，対応ありのWilcoxon符号付き順位検定（両側）を行った．ウェブ高はRAGありで平均2,408\,mm，RAGなしで2,119\,mmであり，13.6\%の増加が高度に有意であった（$p = 1.4 \times 10^{-6}$）．上フランジ幅（+22.5\%，$p = 7.5 \times 10^{-6}$）および下フランジ幅（+20.0\%，$p = 1.7 \times 10^{-5}$）も有意に増加した．一方，桁本数には有意差がなかった（$-0.9\%$）．
...
\hline
ウェブ高 & 2,408\,mm & 2,119\,mm & +13.6\% & $1.4 \times 10^{-6}$ \\
\hline
上フランジ幅 & 542\,mm & 443\,mm & +22.5\% & $7.5 \times 10^{-6}$ \\
\hline
下フランジ幅 & 637\,mm & 531\,mm & +20.0\% & $1.7 \times 10^{-5}$ \\
\hline
桁本数 & 4.0本 & 4.0本 & $-0.9\%$ & --- \\
\hline
\end{tabularx}
...
この因果チェーンは，deflectionの初回合格率改善（+44.8ポイント）が最も顕著であったことと整合する．deflection初回合格群と不合格群のウェブ高中央値を比較すると，RAGあり合格群は2,800\,mm，RAGあり不合格群は1,900\,mmであり，900\,mmの差が存在する．$I$はウェブ高の3乗に比例するため，この差がたわみ照査の合否に決定的に作用している．
...
LLMが5本桁を選択する背景には，RAGログのreasoningに記録された以下の判断がある．(1)「5本$\rightarrow$間隔5500\,mmは妥当な範囲（3000--6000\,mm）」，(2)「6本にすると鋼重・製作コストが増加する」，(3)「幅員24\,mクラスでは5主桁程度が標準的」．すなわち，LLMは経済性・標準慣行を優先した結果，構造的に不足する配置を選択している．LLMは曲げ照査の定量的な見通しを持たないまま桁本数を決定しているため，経済性を重視した結果として構造的に過小な配置となってしまう．
...
\hline
5 & 3900$\times$30 & 650$\times$52 & 650$\times$55 & 420 & 1.09 & web幅厚比ギリギリ \\
\hline
\end{tabular}
\end{table*}
...
OpenAI: Structured model outputs, OpenAI API Documentation, \url{https://platform.openai.com/docs/guides/structured-outputs} (accessed 2026-01-28).
...
pdfplumber: pdfplumber - Plumb a PDF for detailed information about each text character, rectangle, and line, \url{https://github.com/jsvine/pdfplumber} (accessed 2026-01-26).
\bibitem{openai_embeddings}
OpenAI: Embeddings - OpenAI API Documentation, \url{https://platform.openai.com/docs/guides/embeddings} (accessed 2026-01-26).
\bibitem{numpy_docs}
NumPy: NumPy Documentation - The fundamental package for scientific computing with Python, \url{https://numpy.org/doc/} (accessed 2026-01-26).
\bibitem{openai_responses}
OpenAI: Responses API - OpenAI API Documentation, \url{https://platform.openai.com/docs/api-reference/responses} (accessed 2026-01-26).
\bibitem{ifcopenshell_docs}
IfcOpenShell: IfcOpenShell Documentation, \url{https://ifcopenshell.org/} (accessed 2026-01-26).
\end{thebibliography}
