...
|-------------|------|----------|----------|---------|
| **せん断** | 100.0% | 100.0% | 100.0% | ± 0.0% |
| **web厚み** | 100.0% | 100.0% | 100.0% | ± 0.0% |
| **床版** | 83.3% | 100.0% | 66.7% | **+33.3%** |
| **曲げ** | 30.2% | 46.9% | 13.5% | **+33.4%** |
| **たわみ** | 28.6% | 51.0% | 6.2% | **+44.8%** |
...
1. **広幅員（B≥20m）への対応**
   ...
   **修正ループの推移: なぜ断面増だけでは収束困難か（L60\_B24\_rag\_true\_trial\_1）**

   このケースでは5桁・桁間隔5500mmで設計が開始される。中間桁の受け持ち幅 b\_i = 5.5m、実効幅 b\_eff = 5.5m（主載荷幅上限と一致）となり、1本あたりの活荷重曲げモーメントが大きくなる。以下に iter0（初期設計）から iter5（最終）までの推移を示す:

   | iter | web (mm) | TF幅×厚 | BF幅×厚 | I (×10⁹mm⁴) | M\_total (GN·mm) | util\_bend | 制約 |
   |------|----------|---------|---------|-------------|-----------------|-----------|------|
   | 0 | 3200×28 | 450×40 | 600×55 | 207 | 38.5 | **1.79** | — |
   | 1 | 3500×28 | 450×44 | 600×55 | 263 | 38.9 | **1.63** | TF厚>40mm → fy 315→295 |
   | 2 | 3700×28 | 450×48 | 600×55 | 308 | 39.1 | **1.46** | web幅厚比 NG (util=1.02) |
   | 3 | 3700×30 | 550×50 | 600×55 | 339 | 39.6 | **1.29** | web厚増で対応 |
   | 4 | 3700×30 | 650×50 | 650×55 | 367 | 39.9 | **1.19** | — |
   | 5 | 3900×30 | 650×52 | 650×55 | 420 | 40.1 | **1.09** | web幅厚比 ギリギリ (util=1.00) |

   5回の修正で I を **2倍以上** に増やした（207→420）にもかかわらず、util\_bend は 1.79→1.09 にとどまり合格（≤1.0）に届かなかった。収束を阻む要因は以下の連鎖にある:

   1. **受け持ち幅が大きい**: 桁間隔5500mm → b\_eff = 5.5m → M\_total ≈ 38.5 GN·mm と高止まり
   2. **断面増→自重増の悪循環**: 鋼断面積が増えるたび M\_total が微増（38.5→40.1, +4%）し、改善効果を一部相殺
   3. **腹板幅厚比の制約**: 桁高を上げると web\_height/130 が web\_thickness を超え（iter2）、web厚も増やす必要が生じる
   4. **降伏点の低下**: フランジ厚が40mmを超えると SM490 の fy が 315→295 N/mm² に低下し（iter1）、許容応力が 189→177 N/mm² に **6.3%減少**

   これらの制約が重なるため、許容される刻み幅（ウェブ高 +100〜500mm、フランジ厚 +2〜6mm）では5回の修正で収束できない。根本的には桁本数の増加（5→6本以上）による桁間隔の縮小が必要であるが、現行の修正ループでは桁本数を変更できない。
...
各条件（橋長×幅員）における RAG あり/なしの詳細統計。各条件 n=3（3trial）。
