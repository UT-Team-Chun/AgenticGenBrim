...
| **初回合格率** | 18.8% | **37.5%** | 0.0% |
| **収束率** | 80.2% | **85.4%** | 75.0% |
| **平均イテレーション数** | 1.93 | **1.40** | 2.53 |
| 最終合格率 | 80.2% | 85.4% | 75.0% |
...
1. **RAG の効果**
   ...
   - 平均イテレーション数が改善（2.53 → 1.40）

2. **RAG なしでは初回合格が不可能**
   - RAG なしの初回合格率は **0.0%**（96件全て初回不合格）
   - RAG が初回設計品質に決定的な影響を与えている
...
- **shear（せん断）** / **web_slenderness（腹板幅厚比）**: 全ケースで初回合格（100%）
- **deck（床版厚）**: RAG により 33.3% 改善。RAG ありでは **100%** が初回合格
- **deflection（たわみ）**: 最も RAG の効果が顕著（+44.8%）。RAG なしでは 6.2% と極めて低い
...
- web_slenderness は広幅員 (B=20-24m) で bend と併発する傾向
- 最終利用率が 1.0-1.1 の範囲のケースが多く（38件中24件）、追加イテレーションで収束する可能性がある
...
2. **bend と deflection の同時不合格**
   ...
   - 修正（主にウェブ高増加）はたわみに強く効き曲げに弱く効くため、たわみは早期に解消するが曲げの収束に多くのイテレーションを要する
   - 不収束ケースの大半は deflection 解消済みで bend だけが 1.0 を割れずに終了するパターン
   - 根拠データ: `data/evaluation_v4/results/L*_B*_*.json`（per_check_first_pass, first_utilization）、`data/generated_judge_json/*_iter*_judge.json`（イテレーションごとの utilization 推移）

3. **収束率の改善余地**
   - 不収束ケースの 63%（24/38件）が最終利用率 1.0-1.1 の範囲
   - イテレーション上限の引き上げ（5→7-10）で改善の余地あり

### 今後の改善案

1. **広幅員対応の強化**: B=20m 以上での多主桁配置の最適化ロジック
2. **bend 収束の加速**: たわみ解消後も bend が残るケースに対し、フランジ幅・板厚増加など曲げに直接効く修正を優先する戦略の導入
3. **最大イテレーション数の引き上げ**: 惜しいケースの救済
...
- **収束判定**
    - 最終利用率 1.0-1.1 で停止したケースが多く、追加イテレーションで収束する可能性
