...
また，LLMを土木分野に適用する際の課題として，専門知識の不足が指摘されている．箱石ら\cite{hakoishi_bert}は，汎用的な事前学習済みモデルであるBERTが土木分野の専門文書では精度が低下することを示し，土木特化の学習データによるfine-tuningの必要性を指摘した．

\subsection{RAG（検索拡張生成）による専門知識活用}

Lewisら\cite{rag_original}は，外部知識ベースから関連情報を検索し，その情報を基にテキストを生成するRAG（Retrieval-Augmented Generation: 検索拡張生成）を提案した．RAGは以下の手順で動作する：(1) 文書を適当な単位（チャンク）に分割し，各チャンクの埋め込みベクトルを事前に計算してインデックスを構築する，(2) 質問が与えられたとき，質問の埋め込みベクトルと類似度の高いチャンクを検索する，(3) 検索されたチャンクをコンテキストとしてLLMに入力し，回答を生成する．

さて、LLMの専門知識対応には、大きく分けてfine-tuningとRAGの2つのアプローチが存在する．Soudaniら\cite{soudani_rag_vs_ft}は，低頻度かつ専門的な知識に対してはRAGがfine-tuningを大幅に上回る性能を示すことを報告した．道路橋示方書のような専門的な設計規準は学習データ中の出現頻度が低いため，RAGによるアプローチが適していると考えられる．また，fine-tuningは大量の学習データと計算リソースを必要とするのに対し，RAGはインデックス構築のみで対応可能であり，実用面でも優位性がある．
