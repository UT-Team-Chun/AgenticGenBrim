...
床版はIfcBeamエンティティとして生成し，形状表現にはBrep（Boundary Representation：境界表現）を使用する．床版の外形を定義する頂点座標から閉じた多面体（IfcFacetedBrep）を生成することで直方体を表現する．
...
横桁も同様にIfcBeamエンティティとBrep形状表現で生成する．横桁は隣接する主桁間を橋軸直角方向（幅員方向）に接続する形で配置される．
